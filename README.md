...
**Log Aggregator** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Log-Aggregator --path ./target

# With options
./Log-Aggregator --path ./target --format json --output report.json

# Verbose mode
./Log-Aggregator --path ./target --verbose
```
...
### Basic scan
```bash
./Log-Aggregator --path ./my-project
```

### JSON report
```bash
./Log-Aggregator --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Log Aggregator
  run: ./Log-Aggregator --path . --format json --output report.json
```
